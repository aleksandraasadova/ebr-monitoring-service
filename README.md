...
**Инженерная:** отработать построение production-grade бэкенда на Go — от чистой архитектуры и SOLID до JWT, PostgreSQL-транзакций и MQTT. Проект намеренно сделан без фреймворков, чтобы понять что происходит под капотом и выработать уверенные решения на уровне дизайна кода.
